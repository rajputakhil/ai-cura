...
## Literature-Dependent Criteria via DSPy (dspy-migration branch)

The literature-dependent ACMG/AMP criteria (PS3, PS4, PM3, PS2_PM6, PP1, PP4,
PVS1_RNA) can be extracted from uploaded papers using a typed DSPy signature
(https://github.com/stanfordnlp/dspy) instead of a hand-written prompt plus
regex-JSON parsing. The model's output is validated against a schema, so a
malformed response gets caught instead of silently producing a wrong result.

This lives on the dspy-migration branch, kept separate from main so the two
approaches can be compared directly. See DSPY_MIGRATION.md for what changed,
how to run it, and the prompt-optimization work still to do.
